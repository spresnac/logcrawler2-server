<x-guest-layout>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="py-6 sm:px-6 bg-white border-b border-gray-200 text-center">
                    <h1 class="font-semibold text-3xl text-gray-800 leading-tight">See all your laravel logs in one place!</h1>
                    <span class="text-sm text-gray-500">With all, we mean all your logs from all your projects 😉</span>
                    <p class="sm:mt-4">
                        Logcrawler collects all your laravel logs from all your different projects into just one place. So, you no more need to connect to your production maschine just to see the logs and you can use one place to view your logs ... even when you are not at your working maschine.<br />
                        Use logcrawler, never miss an error again and save time.
                    </p>
                </div>
            </div>

            <div class="mt-6 bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="py-6 sm:px-6 bg-white border-b border-gray-200 text-center">
                    <h2 class="font-semibold text-2xl text-gray-800 leading-tight">Want to have your own log central?</h2>
                    <p class="mb-6">
                        Head over to the repo at github and get your own logcrawler server 💪
                    </p>
                    <div class="flex justify-center mb-12 md:mb-0">
                        <a class="bg-sky-700 hover:bg-sky-600 text-white font-bold py-4 px-6 rounded shadow-lg"
                           href="https://github.com/spresnac/logcrawler2-server">Clone and start <i class="fab fa-git-alt"></i></a>
                    </div>
                </div>
            </div>

            <div class="mt-6 bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="py-6 sm:px-6 bg-white border-b border-gray-200 text-center">
                    <h2 class="font-semibold text-2xl text-gray-800 leading-tight">Want to give something back?</h2>
                    <p class="mb-6">
                        I would be very thankfull for any help. Logcrawler is free and open source, so you can clone your copy and modify it; you can even send your improvements back as pull request. This way, you help logcrawler grow.<br>
                        Don't forget to give logcrawler a star on github to grow it's popularity 😉
                    </p>
                    <p class="mb-6">
                        If coding is not your thing, you can donate something to me. There are several ways to do this, the most common could be to participate in sponsoring me on github.
                    </p>
                    <div class="flex justify-center mb-12 md:mb-0">
                        <a class="bg-sky-700 hover:bg-sky-600 text-white font-bold py-4 px-6 rounded shadow-lg"
                           href="https://github.com/sponsors/spresnac"><i class="fab fa-github"></i> Sponsor me on github</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</x-guest-layout>
